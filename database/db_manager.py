...
class DatabaseManager:
    ...
    def process_match_data(self, match_data: Dict[str, Any]):
        """Processes and saves match data to the PostgreSQL database."""
        ...
        try:
            ...
            if not all([home_player_data, away_player_data, winner_code is not None]):
                logger.warning(f"Skipping match ID {match_data.get('id', 'N/A')} due to incomplete data.")
                return
            ...
            self._insert_match(session, match_data, home_player_id, away_player_id, winner_id)
            self._update_head_to_head(session, winner_id, loser_id, match_data)
    ...
    def _insert_match(self, session, match_data: Dict[str, Any], p1_id: int, p2_id: int, win_id: int):
        stmt = text("""
        ...
        """)
        params = {
            "api_match_id": match_data.get("id"), "p1_id": p1_id, "p2_id": p2_id, "win_id": win_id,
    ...
    def _update_head_to_head(self, session, winner_id: int, loser_id: int, match_data: Dict[str, Any]):
        ...
        p2_wins_inc = 1 if winner_id != p1_id else 0  # Corrected logic

        if h2h_id_result:  # UPDATE
            stmt = text(f"""
                UPDATE head_to_head SET
                    ...
                    last_match_winner_id = :winner_id,
    ...
    def close(self):
        # SQLAlchemy manages connections via pooling, so an explicit close is less critical
        # but closing the engine disposes of the connection pool.
